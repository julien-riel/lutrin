#!/usr/bin/env node
/**
 * Keeps each skill's `SKILL.md` in ONE place and copies it wherever a host
 * expects to find it.
 *
 * A skill is authored once, in `packages/core/skills/<name>/SKILL.md` — beside
 ...
 *
 *   - `.claude/skills/<name>/SKILL.md` — the Claude Code skill (this repo's own
 *     `.claude/` tree);
 *   - `plugin/skills/<name>/SKILL.md` — the portable Agent Plugin
 *     (agent-plugins.org §6.1 fixes `skills/<name>/SKILL.md` as the discovery
 *     location).
 *
 ...
 *
 * Usage:
 *   node scripts/sync-skill.mjs           # write the copies from the canonical files
...
/** Every skill this repository ships. Adding one here is the whole
 *  registration: the sync, the anti-drift test and the plugin discovery-path
 *  test all read this list. */
export const SKILL_NAMES = ['deck', 'kit-from-site'];

/** One entry per skill: the authored copy and the generated ones, each at the
 *  fixed path its host discovers it under. */
export const SKILLS = SKILL_NAMES.map((name) => ({
  name,
  canonical: path.join(ROOT, 'packages', 'core', 'skills', name, 'SKILL.md'),
  copies: [
...
function main() {
  ...
  for (const skill of SKILLS) {
    const source = fs.readFileSync(skill.canonical);
    for (const dest of skill.copies) {
      copies += 1;
  ...
  if (check) {
    if (drifted.length) {
      console.error(
        `✖ SKILL.md copies out of sync with packages/core/skills/:\n${drifted
          .map((d) => `  - ${rel(d)}`)
          .join('\n')}\n  Run: node scripts/sync-skill.mjs`,
      );
      process.exit(1);
    }
    console.log(`✓ SKILL.md in sync across ${copies} copies (${SKILLS.length} skills)`);
    return;
  }

  if (drifted.length) for (const d of drifted) console.log(`✓ wrote ${rel(d)}`);
  else console.log(`✓ SKILL.md already in sync (${copies} copies, ${SKILLS.length} skills)`);
}

// Run only when invoked directly (`node scripts/sync-skill.mjs`), not when a
// test imports SKILLS — importing must not sync the tree as a side effect.
// Node 22 has no stable `import.meta.main`, hence the argv comparison.
