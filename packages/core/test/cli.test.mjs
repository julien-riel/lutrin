...
test('build refuses an output whose extension does not match the format asked for', (t) => {
  ...
  const r = lutrin(['build', src, '--pdf', '-o', path.join(dir, 'out.txt')]);
  assert.notEqual(r.code, 0);
  assert.match(r.stderr, /must have the \.pdf extension/);
...
test('--pdf, --png and --jpeg are documented flags of build', () => {
  const r = lutrin(['--help']);
  for (const flag of ['--pdf', '--png', '--jpeg'])
    assert.match(r.stdout, new RegExp(`lutrin build [^\\n]*${flag}`), `${flag} undocumented`);
});
