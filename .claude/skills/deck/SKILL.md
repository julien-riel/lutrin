...
| `apex` | 2 to 6 | levels stacked into a triangle, the apex first — proportions, priorities. NOT `pyramid`, which is the official layout of bands that carry a heading *and* its paragraph |
...
**Real SmartArt, by default.** Those five export as genuine OOXML SmartArt:
PowerPoint opens its own *SmartArt Design* ribbon on the object. It costs
something, and the cost decides when to opt out — **Keynote and macOS Quick
Look show nothing**, PowerPoint re-lays the diagram out with its own engine (so
the frame, the node count, the reading order and the palette are guaranteed,
but not the coordinates), and the object is not animated.

`lutrin build deck.md --no-smartart` (or `smartart: false` in the frontmatter)
draws the diagram as native editable shapes instead: pixel-identical to the
HTML, animatable, and visible in every reader. **Reach for it when the deck
will be opened in Keynote or previewed in Quick Look, or when the slide must
match the HTML exactly.** Either way the setting is ignored on the HTML path.
