...
everywhere the DSL asks for a name from a closed list nobody remembers in
full. `<!-- l` offers the directive keys and chains into their values;
`<!-- layout: ` lists the layouts with their catalogue descriptions;
`<!-- animate: ` (and the frontmatter's `animate:`) the presets plus
`true`/`none`; `](lucide:` the icon set, its ~2 000 names fetched once from
an index both site generators publish beside the icons (the layouts.json
...
- **Kits load for real.** The picker fetches an example kit's files (declared
  by `/kits/index.json` — generated by `site-serve.mjs`, written by
  `pages.yml`, the two pinned to each other by a test) into the virtual
  filesystem and hands the compiler a `themePath` — from there on it is the
  SAME code that resolves a kit directory on disk: manifest read, theme
  validated and confined, fonts embedded, the kit's own layouts registered.
  That last part is the live-list principle paying off: a kit's layouts
  (`lede`, `for-against`…) appear in the layout completion the moment the kit
  is active, because the completion reads the registry, not a list. The
  choice persists with the rest of the workspace and restores BEFORE the
  first compile, so a branded deck reopens branded. `path.normalize` joined
